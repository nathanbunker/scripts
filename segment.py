...
def generate_mapping(root):
    ...
    for message in root.findall('.//Message'):
        ...
        for group in message.findall('./Group'):
            ...
            group_usage_desc = {'R': 'required', 'RE': 'required, when information is available', 'O': 'optional'}.get(group_usage, group_usage)
            ...
            for segment in group.findall('./Segment'):
                ...
                for sg in group.findall('./Group'):
                    ...
                    sg_usage_desc = {'R': 'required', 'RE': 'required, when information is available', 'O': 'optional'}.get(sg_usage, sg_usage)
...
# Replace 'your_file_path.xml' with the actual file path
root = parse_xml('VXu-Z22-Profile.xml')
mappings = generate_mapping(root)
